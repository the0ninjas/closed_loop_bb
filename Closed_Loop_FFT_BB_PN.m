function Closed_Loop_FFT_BB_PN()
...
%% Setup continuous background pink noise
try
    % Create continuous pink noise (60 seconds long to minimize looping artifacts)
    continuous_duration = 60; % seconds
    t_continuous = (0:1/audio_fs:continuous_duration-1/audio_fs)';
    n_continuous = length(t_continuous);
    
    % Generate pink noise for background
    background_amplitude = 0.15; % Lower amplitude for background noise
    
    if exist('dsp.ColoredNoise', 'file')
        % Use DSP toolbox if available
        pink_gen = dsp.ColoredNoise('Color', 'pink', 'SamplesPerFrame', n_continuous);
        background_left = step(pink_gen) * background_amplitude;
        background_right = step(pink_gen) * background_amplitude;
    else
        % Alternative method without DSP toolbox
        white_noise_left = randn(n_continuous, 1);
        white_noise_right = randn(n_continuous, 1);
        
        % Apply pink noise filter
        b = [0.049922035, -0.095993537, 0.050612699, -0.004408786];
        a = [1, -2.494956002, 2.017265875, -0.522189400];
        
        background_left = filter(b, a, white_noise_left) * background_amplitude;
        background_right = filter(b, a, white_noise_right) * background_amplitude;
        
        % Normalize
        background_left = background_left / max(abs(background_left)) * background_amplitude;
        background_right = background_right / max(abs(background_right)) * background_amplitude;
    end
    
    % Create stereo background noise
    background_stereo = [background_left, background_right];
    
    % Create continuous background player
    background_player = audioplayer(background_stereo, audio_fs);
    
    disp('Background pink noise player created successfully');
catch e
    warning('Failed to create background pink noise: %s', e.message);
    background_player = [];
end
...
% Start continuous background pink noise
if exist('background_player', 'var') && ~isempty(background_player)
    play(background_player);
    disp('Starting continuous background pink noise');
end
